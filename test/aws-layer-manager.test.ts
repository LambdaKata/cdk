    ...
    describe('pagination with retry logic', () => {
        ...
        it('should clear results on pagination retry', async () => {
            ...
            (paginateListLayers as jest.Mock).mockImplementation(() => {
                callCount++;
                if (callCount === 1) {
                    // Return partial results then throw
                    return {
    ...
    describe('enhanced layer size validation and compression optimization', () => {
        ...
        describe('validateLayerSize', () => {
            it('should pass validation for layers within size limits', async () => {
                // Mock fs.stat to return reasonable size
            ...
            it('should throw LAYER_SIZE_EXCEEDED for oversized ZIP', async () => {
                // Mock fs.stat to return oversized ZIP
                const mockStats = { size: 60 * 1024 * 1024 }; // 60MB (exceeds 50MB limit)
                jest.doMock('fs', () => ({
                    promises: {
                        stat: jest.fn().mockResolvedValue(mockStats),
                    },
                ...
                await expect(validateLayerSize(mockZipPath))
                    ...
                    .toMatchObject({
                        ...
                        message: expect.stringContaining('57.22 MB) exceeds AWS Lambda limit (47.68 MB)'),
                    });
            });

            it('should throw LAYER_SIZE_EXCEEDED for oversized unzipped content', async () => {
                // Mock fs.stat to return reasonable zipped size
                const mockStats = { size: 10 * 1024 * 1024 }; // 10MB zipped (within limit)
                jest.doMock('fs', () => ({
                    promises: {
                        stat: jest.fn().mockResolvedValue(mockStats),
                    },
                ...
                await expect(validateLayerSize(mockZipPath))
                    ...
                    .toMatchObject({
                        code: ErrorCodes.LAYER_SIZE_EXCEEDED,
                        message: expect.stringContaining('286.10 MB) exceeds AWS Lambda limit (238.42 MB)'),
                    });
            });
            ...
            it('should provide detailed error messages with optimization suggestions', async () => {
                ...
                await expect(validateLayerSize(mockZipPath))
                    .rejects
                    .toThrow(expect.stringContaining('Consider optimizing the layer contents'));
            });
        ...
        describe('createLayerZipArchive with optimization', () => {
            it('should create optimized ZIP with compression metrics', async () => {
                // Mock fs.stat to return final ZIP size
                const mockStats = { size: 5 * 1024 * 1024 }; // 5MB final size
                jest.doMock('fs', () => ({
                    promises: {
                        stat: jest.fn().mockResolvedValue(mockStats),
                    },
            ...
            it('should log detailed compression optimization metrics', async () => {
                ...
                expect(loggerSpy).toHaveBeenCalledWith(
                    ...
                    expect.objectContaining({
                        ...
                        spaceSavedMB: expect.stringContaining('14.31'),
                        finalSizeMB: expect.stringContaining('4.77'),
                    })
                );
            });
                ...
                jest.doMock('fs', () => ({
                    promises: {
                        ...
                        stat: jest.fn()
                            ...
                            .mockResolvedValueOnce({ size: 2048 }), // file2.bin
                    },
                }));

                // Mock recursive call for subdirectory
                const calculateDirectorySize = jest.fn()
                    .mockResolvedValueOnce(512); // Recursive call for subdirectory

                // Replace the method temporarily
                const originalMethod = (manager as any).calculateDirectorySize;
                (manager as any).calculateDirectorySize = calculateDirectorySize;

                const result = await calculateDirectorySize(mockTempDir);

                expect(result).toBe(512); // Just the mocked recursive result

                // Restore original method
                (manager as any).calculateDirectorySize = originalMethod;
            });
