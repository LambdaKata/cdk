...
- [x] 1. Add esbuild configuration for snapstart handler
  - [x] 1.1 Add `snapstartHandler` build target to `utils/esbuild/esbuild.config.ts`
  ...
  - [x] 1.2 Ensure `snapstartHandler` is included in default build (not filtered by stageFilters)
    - Verify the target key is not in the filter exclusion list
    - _Requirements: 1.4_

- [x] 2. Checkpoint - Verify build produces handler bundle
  - Run `yarn build` and verify `out/dist/snapstart-handler.js` exists
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Refactor snapstart-construct.ts to use asset-based code
  - [x] 3.1 Add `path` import to `snapstart-construct.ts`
    - Import `path` from Node.js built-ins
    - _Requirements: 3.3_
  
  - [x] 3.2 Remove `generateHandlerCode()` method entirely
    - Delete the entire method (~120 lines)
    - _Requirements: 3.1_
  
  - [x] 3.3 Modify `createProviderFunction()` to use `Code.fromAsset()`
...
- [x] 4. Checkpoint - Verify CDK synthesis works
  - Run a test CDK synth to verify the construct synthesizes correctly
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Update snapstart-construct tests
  - [x] 5.1 Update test expectations for asset-based code
  ...
  - [x] 5.2 Write property test for asset-based code deployment
    - **Property 1: Asset-Based Code Deployment**
    - **Validates: Requirements 3.2, 3.4, 3.5**
  
  - [x] 5.3 Write property test for backward compatible API
    - **Property 3: Backward Compatible API**
    - **Validates: Requirements 5.1, 5.2**

- [x] 6. Add build verification tests
  - [x] 6.1 Create `test/snapstart-handler-build.test.ts`
  ...
  - [x] 6.2 Write property test for handler bundle exports
